 ...
 * @copyright 	Copyright (c) 2012, PROTOS.
 * @license  	GLP
 * @since  		Version 1.0
 * @version  	1.0
 */
...
*2)Create a folder in with the name [css] at ./ location.
...
class Less_css {
	...
	public function __construct($props = array())
	{
		...
		log_message('debug', "Less_css Class Initialized");
	}
	// --------------------------------------------------------------------

	/**
	 * initialize Less_css preferences
	 *
	 * @access	public
	 * @param	array
	 * @return	bool
	...
	{
		#The css class config's preferences-->
		$config["css_path"]	= ' ';
	...
	function parse(){
		if(file_exists($this->config['css_path']) && !is_dir($this->config['css_path'])){
			$less 	= new lessc();		
			$_ci_CI =& get_instance();		
			
			if($this->config['compress'] == FALSE){
				@$_ci_CI->output->set_output($less->parse(file_get_contents($this->config['css_path'])));			
			}else{
				@$_ci_CI->output->set_output($this->__compress($less->parse(file_get_contents($this->config['css_path']))));
			}
		$this->css_string=$less->css_string;
		}else{
			show_404();
		}
	}
 ...
 */
 class lessc {
	...
	function compileBlock($block, $parent_tags = null) {
		...
		$this->css_string=$this->formatter->css_string;
	...
	private function addZero($input)
	{
		$output=$input;
		if(strlen($input)==1)
		{
	...
	private function ReturnTrueRGB($R,$G,$B)
	{
		$R_=$this->addZero(sprintf("%X",$R));
		$G_=$this->addZero(sprintf("%X",$G));
		$B_=$this->addZero(sprintf("%X",$B));
	...
	function compileValue($value) {
		...
		case 'color':
			...
			$h =$this->ReturnTrueRGB($r,$g,$b);//sprintf("#%x%x%x",$r,$g,$b); //sprintf("#%x%x%x",$r,$g,$b);
...
class lessc_formatter {
	...
	function block($tags, $wrapChildren, $lines, $children, $indentLevel) 
	{
		...
		if ($showDelim) {
			...
			$this->css_string.=$indent.$tags;
			if ($isSingle) $this->css_string.=$this->openSingle;
			else {
				$this->css_string.=$this->open;
				if (!empty($lines)) $this->css_string.=$nl;
				else $this->css_string.=$this->break;
			}
		}

		$this->css_string.=implode($nl, $lines);

		if ($wrapChildren) {
			if (!empty($lines)) $this->css_string.=$this->break;
			foreach ($children as $child) $this->css_string.=$child;
		}

		if ($showDelim) {
			if ($isSingle) $this->css_string.=$this->closeSingle;
			else {
				if (!$wrapChildren) $this->css_string.=$this->break;
				$this->css_string.=$indent.$this->close;
			}
			$this->css_string.=$this->break;
		} elseif (!empty($lines)) {
			$this->css_string.=$this->break;
		}
